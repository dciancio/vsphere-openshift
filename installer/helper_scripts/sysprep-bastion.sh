...
yum install -y lvm2 wget git net-tools bind-utils yum-utils iptables-services bridge-utils bash-completion kexec-tools sos psacct unzip socat

sed -i 's/^#compress/compress/g' /etc/logrotate.conf

su - cloud-user bash -c "cat >~/ansible.cfg <<EOF
[defaults]
...
su - cloud-user bash -c "[ -h ~/openshift-ansible ] || ln -s /usr/share/ansible/openshift-ansible"

su - cloud-user bash -c "curl https://get.acme.sh | sh"

# Loop until all nodes syspreps have completed
echo "WAITING FOR NODE SYSPREP TO COMPLETE..."
timeout 1h bash -c "until su - cloud-user bash -c \"ansible -i hosts nodes -m shell -a 'cat /root/sysprep_complete.txt' &>/dev/null\"; do echo \"Waiting 60s...\"; sleep 60; done"
